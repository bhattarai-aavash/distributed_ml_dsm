...
class DistributedCICSeqMLPTrainer:
    """Sequential-consistency flavored MLP trainer for CIC-IDS2017 using KeyDB."""
    ...
    def __init__(self, keydb_host='localhost', keydb_port=6379, keydb_db=0, server_id='unknown'):
        ...
        self.log_file = f'logs/cic_seq_training_log_{server_id}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        self._init_log_file()
    ...
    def save_weights(self) -> bool:
        """Save weights with Redis WAIT for replica acknowledgment (sequential-ish update)."""
        attempt = 0
        while True:
            attempt += 1
    ...
    def train_distributed(self,
                          ...
                          dropout_p: float = 0.1,
                          eval_every: int = 50):
        print(f"[{self.server_id}] 🚀 Starting CIC-IDS2017 sequential distributed training on local shard...")
...
def main():
    ...
    print(f"🚀 Starting CIC-IDS2017 sequential distributed MLP trainer on server: {server_id}")
    trainer = DistributedCICSeqMLPTrainer(server_id=server_id)
    ...
    if success:
        print(f"[{server_id}] ✅ CIC-IDS2017 sequential distributed training completed successfully!")
    else:
        print(f"[{server_id}] ⚠️  CIC-IDS2017 sequential distributed training stopped without reaching target loss")
