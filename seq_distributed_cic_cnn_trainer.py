...
class DistributedCICSeqCNNTrainer:
    def __init__(self, keydb_host='localhost', keydb_port=6379, keydb_db=0, server_id='unknown'):
        ...
        self.log_file = f'logs/cic_cnn_seq_training_log_{server_id}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        self._init_log_file()
    ...
    def save_weights(self) -> bool:
        # Sequential-like save with WAIT (2 replicas, 10s)
        attempt = 0
        while True:
            attempt += 1
            ...
            try:
                ...
                if wait_result >= 2:
                    print(f"[{self.server_id}] ✅ Weights saved; {wait_result} replicas acknowledged")
                    return True
                print(f"[{self.server_id}] ⚠️ Only {wait_result} replicas acknowledged; retrying...")
                time.sleep(1)
            except Exception as e:
                print(f"[{self.server_id}] ❌ Error saving weights: {e}; retrying...")
                continue
    ...
    def _extract_model_weights(self):
        ...
        for name, module in [("conv1", self.model.conv1), ("conv2", self.model.conv2), ("conv3", self.model.conv3), ("conv4", self.model.conv4)]:
            extracted[f"{name}_weights"] = module.weight.detach().to('cpu').numpy()
            extracted[f"{name}_bias"] = module.bias.detach().to('cpu').numpy()
        for name, module in [("fc1", self.model.fc1), ("fc2", self.model.fc2), ("fc3", self.model.fc3)]:
            extracted[f"{name}_weights"] = module.weight.detach().to('cpu').numpy().T
            extracted[f"{name}_bias"] = module.bias.detach().to('cpu').numpy()
    ...
    def train_distributed(self,
                          ...
                          dropout_p: float = 0.1,
                          eval_every: int = 50):
        print(f"[{self.server_id}] 🚀 Starting CIC CNN sequential distributed training on local shard...")
...
def main():
    ...
    print(f"🚀 Starting CIC CNN sequential distributed trainer on server: {server_id}")
    trainer = DistributedCICSeqCNNTrainer(server_id=server_id)
