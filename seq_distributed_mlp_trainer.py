...
class DistributedMLPTrainer:
    """Distributed MLP trainer using KeyDB for shared weight storage across servers"""
    ...
    def __init__(self, keydb_host='localhost', keydb_port=6379, keydb_db=0, server_id='unknown'):
        ...
        self.experiment_name = "amber_mlp"
    ...
    def train_distributed(self, 
                         ...
                         dropout_p: float = 0.1,
                         eval_every: int = 50):
        """Train on local shard data until target loss is reached"""
        print(f"[{self.server_id}] 🚀 Starting distributed training on local shard...")
                ...
                if iteration == 1 or (iteration % print_every == 0):
                    ...
                    if iteration % eval_every == 0 or iteration == 1:
                        ...
                        if val_loss <= target_loss:
                            ...
                            # Also write a final row with current best validation metrics
        ...
        # Report completion if max iterations reached
        ...
        print(f"[{self.server_id}] Total training time: {elapsed_time:.2f} seconds")
        # Append final test metrics to CSV as well
        self.log_training_step(
            iteration, epoch, batch_loss, batch_acc,
    ...
    def load_amber_parquet_clean(self, file_path: str) -> Tuple[np.ndarray, np.ndarray]:
        """Load cleaned Parquet shard data"""
...
def main():
    ...
    print(f"🚀 Starting distributed MLP trainer on server: {server_id}")
    
    # Initialize trainer
    trainer = DistributedMLPTrainer(server_id=server_id)
    
    # Check if weights exist in KeyDB
    if not trainer.load_weights():
        print(f"[{server_id}] ❌ No weights found. Please run the initializer first!")
        return
    
    print(f"[{server_id}] ✅ Weights loaded successfully from KeyDB")
    ...
    if success:
        print(f"[{server_id}] ✅ Distributed training completed successfully!")
    else:
        print(f"[{server_id}] ⚠️  Distributed training stopped without reaching target loss")
