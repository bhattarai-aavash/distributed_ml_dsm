...
class DistributedCNN1DTrainer:
    ...
    def save_weights(self) -> bool:
        """Save updated weights back to KeyDB with replica acknowledgment using WAIT command"""
        attempt = 0
        
        while True:  # Indefinite loop until success
            attempt += 1
            print(f"[{self.server_id}] 🔄 Weight update attempt {attempt}")
            ...
            try:
                ...
                # Wait for acknowledgment from all replicas (assuming 3 servers total)
                # WAIT command: wait for 2 replicas to acknowledge within 10 seconds
                wait_result = self.redis_client.execute_command('WAIT', 2, 10000)  
                
                if wait_result >= 2:
                    print(f"[{self.server_id}] ✅ Weights saved successfully! {wait_result} replicas acknowledged")
                    return True
                else:
                    print(f"[{self.server_id}] ⚠️ Only {wait_result} replicas acknowledged within timeout")
                    print(f"[{self.server_id}] 🔄 Retrying weight update...")
                    time.sleep(1)  # Brief delay before retry
                    continue
                    
            except Exception as e:
                print(f"[{self.server_id}] ❌ Error in weight update attempt {attempt}: {e}")
                print(f"[{self.server_id}] 🔄 Retrying weight update...")
    ...
    def _create_new_weights(self):
        """Create new weights for the deep architecture"""
        print(f"[{self.server_id}] 🔨 Initializing new weights for deep 4-layer CNN...")
    ...
    def train_distributed(self, 
        ...
        print(f"[{self.server_id}] 🚀 Starting distributed DEEP 4-layer CNN training on local shard...")
                ...
                if iteration == 1 or (iteration % print_every == 0):
                    ...
                    if iteration % eval_every == 0 or iteration == 1:
                        ...
                        if val_loss <= target_loss:
                            print(f"[{self.server_id}] 🎯 Target loss {target_loss} reached! Stopping training.")
                            final_loss = val_loss
                            final_acc = val_acc
                            ...
                            print(f"[{self.server_id}] 🎉 Training completed early at iteration {iteration}!")
                            print(f"[{self.server_id}] Final validation loss: {final_loss:.6f}")
                            print(f"[{self.server_id}] Final validation accuracy: {final_acc:.4f}")
                            ...
                            print(f"[{self.server_id}] " + "="*60)
                            # Log final test metrics to CSV as well
        ...
        # Training completed after max_iterations
...
def main():
    ...
    print(f"🚀 Starting distributed simple 1D CNN trainer on server: {server_id}")
    ...
    if not trainer.load_weights():
        print(f"[{server_id}] ❌ No weights found. Please run the simple CNN initializer first!")
        return
    
    print(f"[{server_id}] ✅ Simple CNN weights loaded successfully from KeyDB")
    
    # Start distributed training
    try:
        success = trainer.train_distributed(
            target_loss=0.2,
            max_iterations=1000,  # Fixed to 1000 iterations
        ...
        if success:
            print(f"[{server_id}] ✅ Distributed simple CNN training completed successfully!")
        else:
            print(f"[{server_id}] ⚠️  Distributed simple CNN training stopped without reaching target loss")
